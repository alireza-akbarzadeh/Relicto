...
export function StyleProgression({ styles, unlocked }: { styles: ArcanaStyle[]; unlocked: string }) {
  const [active, setActive] = useState(styles.find((style) => style.active)?.id ?? styles[0].id);
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-border-subtle bg-surface-card p-4 shadow-lg">
      ...
      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-3">
        {styles.map((style) => {
          const selected = style.id === active;
          return (
            <Button
              key={style.id}
              variant={null}
            ...
            >
              {selected && <span className="absolute top-0 right-0 h-8 w-8 translate-x-3 -translate-y-3 rotate-45 bg-tertiary/20" />}
              <span className="flex items-center justify-between">
                <span className={cn("font-label-caps text-[11px] font-bold", selected ? "text-tertiary" : "text-text-muted group-hover:text-text-primary")}>
                  {style.label}
                </span>
                <span className={cn("font-data-mono-md text-[10px]", selected ? "font-bold text-tertiary" : REQUIREMENT[style.requirementTone])}>
                  {style.requirement}
                </span>
              </span>
              <span className={cn("mt-1 truncate font-body-sm text-xs", selected ? "font-bold text-text-primary" : "font-medium text-text-secondary")}>
                {style.name}
              </span>
              <span className={cn("mt-2 block font-label-badge text-[10px]", selected ? "font-medium text-tertiary" : "text-text-muted")}>{style.note}</span>
            </Button>
          );
        })}
      </div>
